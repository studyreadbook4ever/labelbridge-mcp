...
npm install
npm run build
PORT=3000 HOST=0.0.0.0 PUBLIC_BASE_URL=http://localhost:3000 npm start
```
...
배포 환경에서는 `PUBLIC_BASE_URL`을 PlayMCP에서 접근 가능한 HTTPS URL로 설정하세요.
...
```bash
npm run build
PORT=3000 HOST=127.0.0.1 PUBLIC_BASE_URL=http://127.0.0.1:3000 npm start
